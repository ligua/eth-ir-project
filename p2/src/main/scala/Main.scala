...
object Main {
  ...
  def runModel() = {
    ...
    val FeatureExtractorUsed = FeatureExtractor

    val extracted = FeatureExtractorUsed.extract_features()
    ...
    for(best1000FeatureForTopic <- FeatureExtractorUsed.best1000FeaturesForRanking)
      {
        ...
        if(topic_counter < 40) {
          ...
          val groundTruth = FeatureExtractorUsed.scoresCollectionSorted
        ...
        val resultList2 = FeatureExtractorUsed.languageModelResultLists(topic_counter+51).toList.map(lmr => lmr.correspondingDoc)
    ...
    writer_stats.println(s"\nMAP score for machine learning term based model: $MAP")
    writer_stats2.println(s"\nMAP score for language based model: $MAP2")
